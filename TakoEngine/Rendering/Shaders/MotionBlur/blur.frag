...
vec4 motion_blur()
{
	vec4 scene = texture(sourceTexture, v_TexCoord);
	float depth = texture(depthTexture, v_TexCoord).a;

	vec4 final = scene;
	vec2 texelSize = 1.0 / vec2(textureSize(velocityTexture, 0));
	float velocityScaler = fps_scaler / target_fps;
	int MAX_SAMPLES = 16;

	vec2 velocity = texture(velocityTexture, v_TexCoord).xy;
	...
	{		
		...
		float sampleDepth = texture(depthTexture, sampleTexCoord).a;
		vec2 sampleVelocity = texture(velocityTexture, sampleTexCoord).xy * velocityScaler;
		...
		vec4 currentSample =  texture(sourceTexture, sampleTexCoord);
